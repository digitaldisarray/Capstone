...
public class Window extends JApplet {
	...
	public static WindowInstance size(int width, int height) {
		...
		synchronized (Window.class) {
			...
			// If there is no instance already running, create one and set it as the current
			// instance.
			if (instance == null) {
	 ...
	 * mesh can also be deployed as a stand-alone program via the Mesh class, which
	 * implements the same basic methods. For the sake of simplicity, keys are
	 * strings, and values are either integers, doubles, or strings.
	 * 
	 * @see Mesh
	 */
	...
	public static class out {
		...
		public static void circle(double x, double y, double radius) {
			if (!initialized)
				Window.size(defaultWidth, defaultHeight);
			Window.getInstanceFromThread().fillOval((int) (x - radius), (int) (y - radius), (int) (radius * 2),
					(int) (radius * 2));
		}
		...
		/**
		 * Fills a rectangle centered at the given x, y coordinate, rotated around its
		 * center by the given angle.
		 * 
		 * @param x      - x coordinate of the rectangle
		 * @param y      - y coordinate of the rectangle
		 * @param width  - width of the rectangle
		...
		public static void rectangle(int x, int y, int width, int height, double angle) {
			...
			Window.out.polygon(x + rotatedX(-width / 2, -height / 2, angle),
					y + rotatedY(-width / 2, -height / 2, angle), x + rotatedX(-width / 2, height / 2, angle),
					y + rotatedY(-width / 2, height / 2, angle), x + rotatedX(width / 2, height / 2, angle),
					y + rotatedY(width / 2, height / 2, angle), x + rotatedX(width / 2, -height / 2, angle),
		 ...
		 * length, rotated by the given angle.
		 * 
		 * @param x     - the x coordinate of the square
		 * @param y     - the y coordinate of the square
		 * @param side  - side length of the square
		...
		/**
		 * Fills the oval centered at the given (x, y) coordinate with the given width
		 * and height.
		 * 
		...
		/**
		 * Draws an arc at the given x and y coordinate with the given width, height,
		 * start angle, and arc angle.
		 * 
		 * @param x          - starting x coordinate
		 * @param y          - starting y coordinate
		...
		/**
		 * Sets the color to the given String color - if the color isn't built in, this
		 * will choose black.
		 * 
		...
		public static void color(String color) {
			...
			if (color != null) {
				...
				if (color.indexOf('#') == 0 && color.length() == 7) {
					try {
						Window.getInstanceFromThread().setColor(Integer.valueOf(color.substring(1, 3), 16),
		 ...
		 * Author: Kevin
		 * 
		 * Sets the color to the given Color object
		 * 
		 * @param color - The color object to read from
		 */
		public static void color(Color color) {
			if (!initialized)
				Window.size(defaultWidth, defaultHeight);
			Window.getInstanceFromThread().setColor(color.getRed(), color.getGreen(), color.getBlue());
		}
		...
		public static void randomColor() {
			if (!initialized)
				Window.size(defaultWidth, defaultHeight);
			Window.getInstanceFromThread().setColor((int) (Math.random() * 256), (int) (Math.random() * 256),
					(int) (Math.random() * 256));
		}
		...
		/**
		 * Draws the image represented by the given path at the given (x, y) coordinate,
	...
	/**
	 * Push the drawn contents to a frame that remains fixed on the viewing screen
	 * until another frame is produced and drawn.
	 * 
	...
	/**
	 * WindowInstance is the underlying representation of a window, and is created
	 * by the Window class through the 'create' method.
	 */
